...
import { RouteLoaderComponent } from './shared/animations/route-loader/route-loader.component';
import { IdleTimeoutService } from './services/idle-timeout.service';
import { TopAnnouncementBarComponent } from './shared/components/top-announcement-bar/top-announcement-bar.component';
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(event => {
      if (event instanceof NavigationStart) {
        const fromLogin = this.router.url === '/login.component' || this.router.url === '/';
        if (fromLogin) return;
